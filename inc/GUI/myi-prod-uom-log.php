<?php
namespace my_inventory\myi_prod_uom_log;
...
if (!is_user_logged_in()) {
    ...
    if (isset($_POST['client-select'])  
        ...
        $my_results = $log->get_prod_uom(  ( count($dt) == 0 ? null : \my_inventory\myi_convert_date($dt[0]) ),
...
?>    
    ...
    if ( $my_results ) {
        foreach ( $my_results as $row ) {
            ...
            echo '<td>' .htmlspecialchars(substr($row->prod_uom,0,strlen($row->prod_uom)-15)) .'</td>';
            echo '<td>' .htmlspecialchars($row->uom_default_level) .'</td>';
            echo '</tr>';
        } // foreach
    } //if
    ?>
