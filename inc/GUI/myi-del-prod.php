<?php
namespace my_inventory\myi_del_prod;
...
if (!is_user_logged_in()) {
    ...
    if (isset($_POST['client-select']) && $_POST['client-select'] != 0 && $_POST['save_not_pressed'] != 'false' 
           && wp_verify_nonce( $_POST['del_prod_nonce'], 'del_prod' ) ) {
            ...
            $result = $client_rec->del_product( get_current_user_id(),
                                               $user_roles->get_saved_role() );
            
                                               
            if ( !$result ) {
                echo '<span id="err_msg" class="glyphicon glyphicon-thumbs-down">    Deletion failed. Ensure that you have rights to delete the product and the product is not already deleted</span><br/>';
            } else {
                echo '<span id="succ_msg" class="glyphicon glyphicon-thumbs-up">    Product deleted...</span><br/>';
            }
    }    
?>
...
<div class="form-group"><pre class="no-display">
    ...
    <?php wp_nonce_field( 'del_prod', 'del_prod_nonce' ); ?>
