<?php
namespace my_inventory\myi_copy_prod_uom;
...
if (!is_user_logged_in()) {
    ...
    if (isset($_POST['client-select']) && $_POST['client-select'] != 0 && $_POST['prod-select'] != 0 && $_POST['save_not_pressed'] != 'false'
           && wp_verify_nonce( $_POST['copy_prod_uom_nonce'], 'copy_prod_uom' ) ) {
            ...
            if ( isset( $_POST['cb'] ) ) {
                foreach( $_POST['cb'] as $id ) {        
                    $uom_rec->get_record_by_id( $id, $user_roles->get_role( get_current_user_id(), $_POST['client-select'] ), 1 ); 

                    $result = $uom_rec->copy_prod_uom( $id, 
                                                       $_POST['copy-client-select' .$id],
                                                       $uom_rec->get_desc(),
                                                       get_current_user_id(),
                                                       $user_roles->get_role( get_current_user_id(), $_POST['client-select'] ),
                                                       $uom_rec->get_uom_default_level() );
                ...
                if ( $cnt_succ <= 0 ) {
                    echo '<span id="err_msg" class="glyphicon glyphicon-thumbs-down">    Copy failed. ' .$cnt_fail .' Product UOMs copy failed. Ensure that you have rights to create and that there is no existing Product UOM for client</span><br/>';
                } else {
                    echo '<span id="succ_msg" class="glyphicon glyphicon-thumbs-up">    ' .$cnt_succ .' Product UOMs copied successfully...' 
                         . ( $cnt_fail > 0 ? $cnt_fail .' Product UOM copying failed...' : '') .'</span><br/>';
                }
            }
    }
?>
...
<div class="form-group"><pre class="no-display">
    ...
    <?php wp_nonce_field( 'copy_prod_uom', 'copy_prod_uom_nonce' ); ?>
          ...
          if ( isset($_POST['client-select']) && $_POST['client-select'] != 0 && $_POST['prod-select'] != 0 ) {
                ...
                <div id="no-more-tables<?php echo $cnt; ?>">
                    <table class="col-md-12 table-bordered table-striped table-condensed cf">
                        ...
                        <tbody>
                        <tr>
                            <td><label class="label_fields">To Copy :</label></td>
                            <td><div class="checkbox">
                                    <label><input type="checkbox" value="<?php echo $prod_uom_recs[$cnt]->id; ?>" id="cb<?php echo $prod_uom_recs[$cnt]->id; ?>" 
                                            name="cb[]"
                                            onchange="cb_change(<?php echo $prod_uom_recs[$cnt]->id; ?>);"></label>
                                </div></td>
                        </tr>   
                        <tr>
                            <td><label class="label_fields">Client to copy to :</label></td>
                            <td><select class="selectpicker click-submit-form" data-style="btn-primary" id="copy-client-select<?php echo $prod_uom_recs[$cnt]->id; ?>" 
                                name="copy-client-select<?php echo $prod_uom_recs[$cnt]->id; ?>" data-live-search="true" 
                            ...
                            for ( $c = 0; $c < count( $clients_arr[0] ); $c++ ) {
                              if ( $clients_arr[0][$c] != $_POST['client-select'] ) {
                                echo '<option data-content="'.htmlspecialchars($clients_arr[1][$c]) .'   ' 
                                      .( $clients_arr[2][$c] == '' ? '' : '(' .htmlspecialchars($clients_arr[2][$c]) .')' ) .'"'
                                      .( isset($_POST['copy-client-select'.$prod_uom_recs[$cnt]->id]) 
                                        && $_POST['copy-client-select'.$prod_uom_recs[$cnt]->id] == $clients_arr[0][$c] ? ' selected=selected ' : '' )
                                        .'>'
                                        .$clients_arr[0][$c] .'</option>';
                              }
                            }
                            ?>
                            </select></td>
                        ...
                        <tr>
                            ...
                            <td><pre><label class="label_values"><?php echo htmlspecialchars($prod_cat_recs[$cnt]->desc); ?></label></pre></td>
                        </tr>
                        <tr>
          ...
          <?php } // for    ?>
                ...
                <div id="no-more-tables-1">
                    <table class="col-md-12 table-bordered table-striped table-condensed cf">
                        ...
                        <tr>
                            <td></td>
                            <td><button type="submit" id="save_btn" class="btn btn-primary"><span class="glyphicon glyphicon-copy">    Copy</span>
                                        </button></td>
                            <td></td>
                        </tr>
                        </tbody>
